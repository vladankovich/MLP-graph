...
| **Funkcija gubitka** | Kategorička unakrsna entropija | $`\mathcal{L}_{\text{CE}}(y, \hat{y}) = -\sum_{k=1}^K y_k \log \hat{y}_k`$ |
...
- Za izlazni sloj ($`l = 3`$): $`\varphi(z)_k = \text{Softmax}(z)_k = \displaystyle\frac{e^{z_k}}{\sum_{j=1}^{10} e^{z_j}}`$
